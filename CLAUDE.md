...
- `nav.js` — single shared script loaded by every page (`<script src="nav.js?v=N">`). Handles: closing nav dropdowns (`details.nav-dropdown`) on outside click/Escape/item-click, custom eased smooth-scrolling for in-page `#anchor` links (with a `prefers-reduced-motion` bypass), scroll-spy nav highlighting via `IntersectionObserver` (homepage only, harmless elsewhere), and an injected contact modal wired to a Formspree endpoint (`data-open-contact` triggers open it). Because `crumple-transition.js` swaps page content in place (no full reload), this script runs only once per tab: all behavior is bound by delegation on `document`/`window` so it survives a swap, and the scroll-spy observer is re-run when crumple-transition.js dispatches a `crumple:swapped` event. Don't reintroduce listeners bound directly to elements inside `.site-wrap` — they'd go dead after the first transition.
...
- `crumple-transition.js` — single shared script loaded by every page (`<script src="crumple-transition.js?v=N" data-base="/assets/" data-swap=".site-wrap" data-sound="on">`). A self-contained, dependency-free newspaper "crumple-and-toss" page transition: it intercepts clicks on internal page-to-page links, crumples the current page into a paper ball, and throws it off-screen to reveal the next page underneath. Runs in seamless `swap` mode (`data-swap=".site-wrap"`): the next page is fetched and its `.site-wrap` contents are swapped in behind the crumple — so the *new* page (not a reload/white flash) is what's revealed as the ball tosses away. After a successful swap it honors a `#hash` in the destination URL (e.g. an article's `index.html#work` link scrolls to that section) and dispatches a `crumple:swapped` event so `nav.js` can re-bind content-tied behavior. Leaves in-page `#anchor` links, downloads (the résumé link), external links, and modified-clicks (cmd/ctrl/new-tab) alone, so there's no conflict with `nav.js`'s smooth-scroll. Respects `prefers-reduced-motion` (skips straight to a normal navigation). Because swap mode keeps the existing `<head>`, all pages must keep loading the same fonts/`styles.css`. On touch devices (`(hover: none) and (pointer: coarse)`) it runs in a "lite" mode that drops the per-frame SVG warp filter (`feTurbulence`/`feDisplacementMap`) — by far the most expensive part, and a major source of jank on high-DPR phones — while keeping the clip-path crush, toss, paper texture, and sound; override with `data-lite="on"`/`"off"` (default `"auto"`).
