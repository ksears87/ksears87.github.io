...
- When editing `nav.js` or `styles.css`, bump the `?v=N` query string on the corresponding `<script>`/`<link>` tag in **every** HTML file that references it, to bust GitHub Pages' cache.
